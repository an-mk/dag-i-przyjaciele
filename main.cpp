...
int main() 
{
	...
	lab.setTestGenerator([](const size_t &sizeOfTest){ return generateRandomDAG(sizeOfTest, 0.3f); });
	...
	lab.setTestGenerator([](const size_t &sizeOfTest){ return generateRandomDAG(sizeOfTest, 0.7f); });
    lab([](const std::pair<adjacencyMatrix, neighbourList>& G){ mst(G.first); }, "mst adjacencyMatrix");
    lab([](const std::pair<adjacencyMatrix, neighbourList>& G){ mst(G.second); }, "mst neighbourList");
    return 0;
}